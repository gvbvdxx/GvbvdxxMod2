...
async function createLogsTab(_ref) {
  ...
  const downloadText = (filename, text) => {
    Object(download_blob["default"])(filename, new Blob([text], {
      type: "text/plain"
    }));
  };
...
var editor_stepping_highlighter = __webpack_require__(2058);
...
async function createThreadsTab(_ref) {
  ...
  const highlighter = new editor_stepping_highlighter["default"](10, "#ff0000");
  logView.generateRow = row => {
    const root = document.createElement("div");
    root.className = "sa-debugger-log";
  ...
  const updateContent = () => {
    ...
    const createThreadInfo = (thread, depth) => {
      ...
      const runningThread = Object(debugger_module["getRunningThread"])();
  ...
  stepButton.element.addEventListener("click", () => {
    Object(debugger_module["singleStep"])();
  });
  const handlePauseChanged = paused => {
    stepButton.element.style.display = paused ? "" : "none";
    updateContent();
  };
  handlePauseChanged(Object(debugger_module["isPaused"])());
  Object(debugger_module["onPauseChanged"])(handlePauseChanged);
  Object(debugger_module["onSingleStep"])(updateContent);
  const show = () => {
    logView.show();
    updateContent();
  };
...
async function createPerformanceTab(_ref) {
  ...
  debug.addAfterStepCallback(() => {
    if (Object(debugger_module["isPaused"])()) {
      return;
    }
    const time = now();
  ...
  let pauseTime = 0;
  Object(debugger_module["onPauseChanged"])(paused => {
    if (paused) {
      pauseTime = now();
    } else {
...
/* harmony default export */ var userscript = (async function (_ref) {
  ...
  const pause = (_, thread) => {
    ...
    Object(debugger_module["setPaused"])(true);
  ...
  unpauseButton.element.addEventListener("click", () => Object(debugger_module["setPaused"])(false));
  const updateUnpauseVisibility = paused => {
    unpauseButton.element.style.display = paused ? "" : "none";
  };
  updateUnpauseVisibility(Object(debugger_module["isPaused"])());
  Object(debugger_module["onPauseChanged"])(updateUnpauseVisibility);
  ...
  const goToBlock = blockId => {
    ...
    new Utils["default"](addon).scrollBlockIntoView(blockId);
  };
...
var style = __webpack_require__(2118);
...
const resources = {
  ...
  "icons\\threads.svg": icons_threads,
